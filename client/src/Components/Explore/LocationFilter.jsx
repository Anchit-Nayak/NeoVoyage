...
const LocationFilter = ({setLocation}) => {
  ...
  const options = ["All Categories", "Mumbai", "Delhi", "Hyderabad"];
        ...
        <ul className="filter-options text-white max-w-md divide-y divide-gray-700">
          {options.map((option, index) => (
            <li
              ...
              className={`${selectedOption === option ? 'selected bg-gray-900' : ''} px-3 py-3 sm:pb-4 hover:bg-gray-900 rounded-lg`}
            >
              {option}
            </li>
          ))}
